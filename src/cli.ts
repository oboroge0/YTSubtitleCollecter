...
program
  ...
  .option(
    '-l, --language <lang>',
    'Subtitle language code (e.g., en, ja, ko)',
    'en'
  )
  ...
  .action(async (url, options) => {
    ...
    try {
      ...
      if (options.language && !validateLanguageCode(options.language)) {
        throw new Error(
          `Invalid language code: ${options.language}. Use ISO 639-1 format (e.g., en, ja, ko).`
        );
      }

      if (options.format && !validateFormat(options.format)) {
        throw new Error(
          `Invalid format: ${options.format}. Valid formats are: txt, srt, vtt, json.`
        );
      }
...
program
  .command('batch <file>')
  .description(
    'Download subtitles from multiple YouTube videos listed in a file'
  )
